...
void ASTNode::ReduceToOneNode()
{
    if (left != nullptr) 
    {
        left->ReduceToOneNode();
    }
    if (right != nullptr) 
    {
        right->ReduceToOneNode();
    }
    if (operation == -1) 
    {
        return;
    ...
    if (operation == ADD) 
    {
        ...
        {
            valoare.real_number = left->valoare.real_number + right->valoare.real_number;
        }
    ...
    else if (operation == BLT) 
    {
        if (left->type == TYPE_INT) 
        {
            valoare.true_or_false = left->valoare.number < right->valoare.number;
        } 
        else 
        {
            valoare.true_or_false = left->valoare.real_number < right->valoare.real_number;
        }
    } 
    else if (operation == BLTE) 
    {
        if (left->type == TYPE_INT) 
        {
            valoare.true_or_false = left->valoare.number <= right->valoare.number;
    ...
    else if (operation == BEQ) 
    {
        if (left->type == TYPE_CHAR) 
        {
            valoare.true_or_false = left->valoare.caracter == right->valoare.caracter;
        } 
        ...
        {
            valoare.true_or_false = !strcmp(left->valoare.string, right->valoare.string);
        } 
